...
Player.prototype = {
  ...
  upgrade_robots: function(request) {
    ...
    if(this.cash >= upgrade_cost) {
      var speed = this.upgrades[request.robot_type].speed;
      ...
      this.upgrades[request.robot_type].speed = Math.min(speed, 8);
  ...
  destroy_robot: function(serial_number) {
    ...
    var cash_value = Math.round(this.robots[position].cash_value, 0);
    if (position) {
      this.robots.slice(position, 1);
