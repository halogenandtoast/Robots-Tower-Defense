...
Player.prototype = {
  new_wave: function(count) {
    ...
    for(var i = 0; i < count; i++) {
      this.wave.push(new Robot(self, 'type1'));
    }
  },
  ...
  fire: function(request) {
    ...
    if(position) {
      var tower = this.towers[this.find_tower_position(request.tower_sn)];
      if(tower) {
        tower.fire(request);
      }
    }
  ...
  upgrade_robots: function(request) {
    ...
    if(this.cash >= upgrade_cost) {
      ...
      this.upgrade_costs[request.robot_type] = this.upgrade_costs[request.robot_type] * 2.25;

      this.relay('cash_amount', { 'amount': this.cash });
      this.relay('robots_upgraded', {
  ...
  fired_heat_seeker: function(data) {
    this.relay('fired_heat_seeker', data);
  },
